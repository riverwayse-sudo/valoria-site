...
export default function MarketplaceExperienceV3({ forcedTrack = 'all', initialRows = [], initialCounts = null }) {
  const [listings, setListings] = useState(initialRows || [])
  const [loading, setLoading] = useState(false)
  const [tab, setTab] = useState(forcedTrack || 'all')
  ...
  const counts = useMemo(() => initialCounts || listings.reduce((result, item) => { if (!item.is_dummy) { result.all += 1; if (result[item.track] != null) result[item.track] += 1 } return result }, { all: 0, candidate: 0, speaker: 0, facilitator: 0 }), [listings, initialCounts])
