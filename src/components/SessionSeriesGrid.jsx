  ...
  return (
    <section className="session-series" id="upcoming-sessions" aria-labelledby="session-series-title">
      <style>{`
        ...
        .session-series-card{min-height:650px;padding:22px 22px 30px;border-right:1px solid rgba(26,26,46,.24);border-bottom:1px solid rgba(26,26,46,.24);display:flex;flex-direction:column;position:relative;transition:transform .35s ease,background .35s ease}.session-series-card:hover{background:rgba(255,255,255,.58);transform:translateY(-5px)}
        .session-series-flyer{position:relative;width:100%;aspect-ratio:3/4;overflow:hidden;background:#e9e5dc;border:1px solid rgba(26,26,46,.12);margin-bottom:24px}.session-series-flyer img{display:block;width:200%;height:100%;max-width:none;object-fit:fill;transition:transform .5s cubic-bezier(.2,.7,.2,1)}.session-series-card:hover .session-series-flyer img{transform:scale(1.018)}
        .session-series-top{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:20px}.session-series-number{font-family:var(--font);font-size:13px;font-weight:800;letter-spacing:.12em;color:#8f6c25}.session-series-status{font-size:10px;font-weight:800;letter-spacing:.11em;color:#414152;text-transform:uppercase;text-align:right}
        .session-series-cluster{font-size:11px;font-weight:800;letter-spacing:.18em;color:#4a4a5a;text-transform:uppercase;margin-bottom:14px}.session-series-card h3{font-family:var(--font);font-size:clamp(26px,2vw,35px);font-weight:500;line-height:1.12;letter-spacing:-.025em;color:#1A1A2E;margin:0}.session-series-description{font-size:16px;line-height:1.65;font-weight:400;color:#343447;margin:18px 0 0}
        .session-series-bottom{margin-top:auto;padding-top:24px}.session-series-date{font-size:11px;font-weight:800;letter-spacing:.1em;color:#4b4b5b;text-transform:uppercase;margin-bottom:16px;line-height:1.55}.session-series-actions{display:flex;gap:10px;align-items:center}.session-series-primary,.session-series-secondary{min-height:50px;padding:0 14px;display:inline-flex;align-items:center;justify-content:center;text-decoration:none;text-transform:uppercase;letter-spacing:.09em;font-size:11px;font-weight:800;border-radius:2px;transition:all .2s ease;cursor:pointer}.session-series-primary{background:#1A1A2E;color:#F7F4EE;border:1px solid #1A1A2E;flex:1}.session-series-primary:hover{background:var(--gold);border-color:var(--gold);color:#1A1A2E}.session-series-secondary{border:1px solid rgba(26,26,46,.32);color:#1A1A2E;background:transparent;flex:1}.session-series-secondary:hover{border-color:var(--gold);color:#76591c}
        ...
        @media(max-width:1100px){.session-series-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.session-series-card{min-height:600px}.session-series-head{align-items:flex-start;flex-direction:column;gap:24px}.session-series-title{font-size:clamp(44px,8vw,68px)}}
        @media(max-width:680px){.session-series{padding-top:78px;padding-bottom:90px}.session-series-intro{font-size:17px}.session-series-grid{display:flex;overflow-x:auto;scroll-snap-type:x mandatory;scrollbar-width:none}.session-series-grid::-webkit-scrollbar{display:none}.session-series-card{min-width:min(84vw,350px);scroll-snap-align:start;padding:18px 18px 26px}.session-series-card h3{font-size:30px}.session-series-description{font-size:16px}.event-modal{padding:28px 22px}.event-modal h2{font-size:34px}.event-countdown-units{gap:5px}}
        @media(prefers-reduced-motion:reduce){.session-series-card,.session-series-flyer img{transition:none}}
      `}</style>
          ...
          {sessions.map((session) => {
            ...
            return (
              <article className="session-series-card" key={session.id}>
                {session.flyerPosition && <div className="session-series-flyer"><img src={EVENT_FLYER_SPRITE} alt={`${session.title} flyer`} style={{ transform: session.flyerPosition === 'right' ? 'translateX(-50%)' : 'translateX(0)' }} /></div>}
                ...
                <div className="session-series-bottom">
                  <div className="session-series-date">{formatSessionDate(session)} · 10:00 AM WAT · VIRTUAL · 90 MINUTES{session.speaker ? ` · ${session.speaker}` : ''}</div>
