...
const COLORS = { gold: '#C9A84C', navy: '#1A1A2E', paper: '#F7F4EE', dark: '#0F0F1A', blue: '#378ADD', teal: '#1D9E75', muted: 'rgba(247,244,238,.58)' }
const TRACKS = {
  ...
  candidate: { label: 'Talent', short: 'Talent', color: COLORS.blue, intro: 'Find assessed professionals for opportunity, placement and strategic introduction.' },
  ...
  facilitator: { label: 'Facilitators', short: 'Facilitators', color: COLORS.teal, intro: 'Find facilitators aligned to PRIME-based capability development and organisational outcomes.' },
}
  ...
  return (
    <main style={S.page}>
      ...
      <section style={S.toolbar} className="marketplace-toolbar">
        <div style={S.searchWrap} className="marketplace-search"><span style={S.searchIcon} aria-hidden="true">⌕</span><input aria-label="Search professionals" style={S.search} placeholder="Search by name, skill, industry or topic…" value={query} onChange={event => setQuery(event.target.value)} /></div>
...
function ProfileCard({ profile: p, currentUser, selected, onCompare }) {
  ...
  return <article className="capability-card" style={S.card}><div style={S.cardAccent}/><div style={S.cardTop}><div style={S.avatar}>{p.photo_url ? <img src={p.photo_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : initials}</div><div style={S.identity}><div style={S.name}>{p.full_name || p.atb_id || 'Valoria Professional'}</div><div style={S.verified}>✓ VALORIA ASSESSED</div><div style={S.headline}>{p.headline || p.designation || 'Valoria Professional'}</div>{p.location && <div style={S.location}>{p.location}</div>}</div>{p.valu_index != null && <div style={S.valu}><span>VALU</span><strong>{p.valu_index}</strong><small>/100</small></div>}</div><div style={S.metaRow}><span style={{ ...S.track, color: meta.color, borderColor: meta.color }}>{meta.label}</span>{availability && <span style={S.status}>● {availability === 'open' ? 'Open' : availability === 'contract_only' ? 'Contract' : availability}</span>}</div>{strengths.length > 0 && <div style={S.strengths}><span>PRIME</span>{strengths.map(item => <b key={item.cluster.letter} title={`${item.cluster.letter}: ${item.value}`}>{item.cluster.letter}</b>)}</div>}{tags.length > 0 && <div style={S.tags}>{tags.map(tag => <span key={tag} style={S.tag}>{tag}</span>)}</div>}{p.bio && <p style={S.bio}>{p.bio.slice(0, 150)}{p.bio.length > 150 ? '…' : ''}</p>}<div style={S.actions}><Link href={`/profile/${p.id}?track=${p.track}`} style={S.view}>VIEW PROFILE →</Link><button type="button" style={{ ...S.compare, ...(selected ? S.compareActive : {}) }} onClick={onCompare}>{selected ? '✓ Comparing' : 'Compare'}</button></div><div style={S.engage}><EnquiryForm professionalProfileId={p.id} atbId={p.atb_id} enquiryType={enquiryType} ctaLabel={cta} currentUser={currentUser} triggerStyle={S.action} /></div></article>
}

function CompareModal({ profiles, close }) { return <div style={S.modalOverlay} onClick={close}><section style={S.modal} onClick={event => event.stopPropagation()}><header style={S.modalHead}><div><div style={S.kicker}>DECISION SUPPORT</div><h2 style={{ margin: '6px 0', fontWeight: 300 }}>Compare professionals</h2></div><button style={S.modalClose} onClick={close} aria-label="Close comparison">×</button></header><div style={S.compareGrid}>{profiles.map(profile => <div key={profile.id} style={S.compareCard}><div style={S.avatar}>{profile.photo_url ? <img src={profile.photo_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : (profile.display_initials || '?')}</div><strong>{profile.full_name || profile.atb_id}</strong><span>{profile.headline || profile.designation || 'Valoria Professional'}</span><b style={{ fontSize: 28, color: COLORS.gold }}>{profile.valu_index ?? '—'}</b><small>VALU Index</small><span style={{ color: COLORS.muted }}>{profile.industry || 'Professional'} · {TRACKS[profile.track]?.label || 'Talent'}</span></div>)}</div><p style={{ color: COLORS.muted, fontSize: 11, lineHeight: 1.7 }}>Review each full profile for experience, PRIME strengths and engagement details before deciding.</p></section></div> }

const S={page:{minHeight:'100vh',background:COLORS.dark,color:COLORS.paper,fontFamily:"'Raleway','Helvetica Neue',Arial,sans-serif",paddingBottom:100},header:{height:72,padding:'0 32px',display:'flex',alignItems:'center',justifyContent:'space-between',background:COLORS.navy,borderBottom:'1px solid rgba(201,168,76,.2)',position:'sticky',top:0,zIndex:20},brand:{display:'flex',flexDirection:'column',alignItems:'center',gap:4},nav:{color:COLORS.muted,textDecoration:'none',fontSize:12},hero:{borderBottom:'1px solid rgba(201,168,76,.12)',background:'radial-gradient(circle at 50% 0%,rgba(201,168,76,.10),transparent 52%)'},heroInner:{maxWidth:1180,margin:'0 auto',padding:'70px 28px 34px'},eyebrow:{color:COLORS.gold,fontSize:9,letterSpacing:'.18em',fontWeight:700,marginBottom:15},h1:{fontSize:'clamp(42px,6vw,76px)',fontWeight:200,lineHeight:1.02,letterSpacing:'-.035em',margin:0,maxWidth:900},heroCopy:{maxWidth:700,color:COLORS.muted,fontSize:14,lineHeight:1.8,margin:'20px 0 28px'},routes:{display:'flex',flexWrap:'wrap',gap:8},route:{display:'flex',gap:10,padding:'10px 15px',border:'1px solid rgba(247,244,238,.12)',borderRadius:999,color:COLORS.muted,textDecoration:'none',fontSize:10,fontWeight:700,textTransform:'uppercase'},routeActive:{background:COLORS.gold,color:COLORS.navy,borderColor:COLORS.gold},toolbar:{maxWidth:1180,margin:'0 auto',padding:'18px 28px',display:'flex',gap:12,alignItems:'center',borderBottom:'1px solid rgba(247,244,238,.08)'},searchWrap:{flex:1,position:'relative'},searchIcon:{position:'absolute',left:14,top:8,color:COLORS.gold,fontSize:20},search:{width:'100%',padding:'13px 15px 13px 40px',boxSizing:'border-box',borderRadius:7,border:'1px solid rgba(201,168,76,.2)',background:'rgba(255,255,255,.035)',color:COLORS.paper,fontFamily:'inherit',outline:'none'},sortLabel:{color:COLORS.muted,fontSize:9,textTransform:'uppercase',letterSpacing:'.1em'},sort:{marginLeft:8,padding:'11px 30px 11px 12px',borderRadius:7,border:'1px solid rgba(201,168,76,.2)',background:COLORS.navy,color:COLORS.paper,fontFamily:'inherit'},mobileFilter:{display:'none'},workspace:{display:'grid',gridTemplateColumns:'240px 1fr',maxWidth:1180,margin:'0 auto'},filters:{padding:'30px 24px',borderRight:'1px solid rgba(201,168,76,.08)'},label:{display:'block',color:COLORS.muted,fontSize:9,textTransform:'uppercase',letterSpacing:'.1em',marginBottom:20},input:{display:'block',width:'100%',marginTop:8,padding:'11px 12px',boxSizing:'border-box',borderRadius:6,border:'1px solid rgba(201,168,76,.16)',background:'rgba(255,255,255,.04)',color:COLORS.paper,fontFamily:'inherit'},primeRow:{display:'flex',gap:6,marginTop:8},prime:{flex:1,padding:'9px 4px',background:'transparent',border:'1px solid',borderRadius:6,cursor:'pointer',fontWeight:700},hint:{fontSize:10,color:'rgba(247,244,238,.35)',lineHeight:1.6,margin:'-8px 0 20px'},clear:{width:'100%',padding:10,background:'transparent',border:'1px solid rgba(201,168,76,.22)',borderRadius:6,color:COLORS.muted,cursor:'pointer'},results:{padding:'32px 28px'},resultsHead:{display:'flex',justifyContent:'space-between',alignItems:'flex-end',gap:30,marginBottom:26},kicker:{fontSize:9,color:COLORS.gold,fontWeight:700,letterSpacing:'.15em'},h2:{fontSize:'clamp(30px,4vw,48px)',fontWeight:200,lineHeight:1.05,margin:'6px 0 0'},resultsCopy:{fontSize:12,color:COLORS.muted,margin:'7px 0 0'},standard:{display:'flex',flexDirection:'column',gap:4,borderLeft:'1px solid rgba(201,168,76,.2)',paddingLeft:16,fontSize:10},grid:{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(290px,1fr))',gap:16},card:{position:'relative',background:COLORS.paper,color:COLORS.navy,padding:22,border:'1px solid rgba(201,168,76,.16)',overflow:'hidden',display:'flex',flexDirection:'column',minHeight:375},cardAccent:{position:'absolute',left:0,top:0,right:0,height:3,background:COLORS.gold,transform:'scaleX(0)',transformOrigin:'left',transition:'transform .35s'},cardTop:{display:'flex',gap:13,alignItems:'flex-start'},avatar:{width:62,height:62,flexShrink:0,background:COLORS.navy,color:COLORS.gold,display:'flex',alignItems:'center',justifyContent:'center',fontSize:18,fontWeight:600,overflow:'hidden',borderRadius:4},identity:{minWidth:0,flex:1},name:{fontSize:17,fontWeight:600,lineHeight:1.2},verified:{fontSize:8,color:'#8A6D25',fontWeight:700,letterSpacing:'.12em',marginTop:5},headline:{fontSize:11,color:'#4E4A43',marginTop:5,lineHeight:1.4},location:{fontSize:10,color:'#77736B',marginTop:3},valu:{display:'flex',flexDirection:'column',alignItems:'flex-end',minWidth:44},metaRow:{display:'flex',alignItems:'center',gap:8,marginTop:18},track:{display:'inline-flex',padding:'5px 8px',border:'1px solid',fontSize:8,fontWeight:700,letterSpacing:'.1em',textTransform:'uppercase'},status:{fontSize:9,color:'#54735E'},strengths:{display:'flex',alignItems:'center',gap:6,marginTop:14,fontSize:8,color:'#77736B',textTransform:'uppercase',letterSpacing:'.08em'},tags:{display:'flex',gap:5,flexWrap:'wrap',marginTop:13},tag:{fontSize:9,padding:'4px 7px',background:'rgba(26,26,46,.07)',color:'#4E4A43'},bio:{fontSize:11,color:'#625E57',lineHeight:1.6,margin:'14px 0 16px'},actions:{display:'flex',justifyContent:'space-between',alignItems:'center',gap:10,marginTop:'auto',paddingTop:14,borderTop:'1px solid rgba(26,26,46,.10)'},view:{color:COLORS.navy,textDecoration:'none',fontSize:9,fontWeight:700,letterSpacing:'.1em'},compare:{background:'transparent',border:0,color:'#77736B',fontSize:9,cursor:'pointer'},compareActive:{color:'#8A6D25',fontWeight:700},engage:{marginTop:10},action:{width:'100%',padding:'10px 12px',background:COLORS.navy,color:COLORS.paper,border:0,borderRadius:2,fontFamily:'inherit',fontSize:9,fontWeight:700,letterSpacing:'.1em',cursor:'pointer'},empty:{padding:'90px 20px',textAlign:'center',color:COLORS.muted,border:'1px dashed rgba(201,168,76,.15)'},emptyMark:{fontSize:26,color:COLORS.gold,marginBottom:12},clearEmpty:{marginTop:18,padding:'9px 14px',background:'transparent',border:'1px solid rgba(201,168,76,.3)',color:COLORS.gold,cursor:'pointer'},skeleton:{background:COLORS.paper,padding:22,minHeight:325,animation:'valoriaPulse 1.4s ease-in-out infinite'},skeletonAvatar:{width:62,height:62,borderRadius:4,background:'rgba(26,26,46,.10)',marginBottom:20},skeletonLine:{height:10,width:'85%',background:'rgba(26,26,46,.10)',marginBottom:12,borderRadius:2},compareBar:{position:'fixed',left:20,right:20,bottom:20,zIndex:40,background:COLORS.navy,border:'1px solid rgba(201,168,76,.35)',boxShadow:'0 18px 60px rgba(0,0,0,.35)',padding:'12px 16px',display:'flex',alignItems:'center',gap:12,fontSize:10},compareNames:{display:'flex',gap:6,flex:1,overflow:'hidden'},compareButton:{background:COLORS.gold,color:COLORS.navy,border:0,padding:'9px 13px',fontSize:9,fontWeight:700,cursor:'pointer'},clearCompare:{background:'transparent',color:COLORS.muted,border:0,cursor:'pointer'},overlay:{position:'fixed',inset:0,zIndex:60,background:'rgba(0,0,0,.65)',display:'flex',alignItems:'flex-end'},sheet:{width:'100%',background:COLORS.navy,padding:'24px 20px 32px',borderTop:'1px solid rgba(201,168,76,.25)'},sheetHead:{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:25,color:COLORS.paper},modalOverlay:{position:'fixed',inset:0,zIndex:70,background:'rgba(0,0,0,.72)',display:'flex',alignItems:'center',justifyContent:'center',padding:20},modal:{width:'min(900px,100%)',maxHeight:'90vh',overflow:'auto',background:COLORS.navy,color:COLORS.paper,border:'1px solid rgba(201,168,76,.3)',padding:28},modalHead:{display:'flex',justifyContent:'space-between'},modalClose:{background:'transparent',border:0,color:COLORS.muted,fontSize:28,cursor:'pointer'},compareGrid:{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12,margin:'22px 0'},compareCard:{background:'rgba(247,244,238,.06)',border:'1px solid rgba(247,244,238,.1)',padding:18,display:'flex',flexDirection:'column',gap:7}}
const CSS=`@keyframes valoriaPulse{0%,100%{opacity:.55}50%{opacity:1}}.capability-card:hover{box-shadow:0 18px 45px rgba(0,0,0,.24)}.capability-card:hover>div:first-child{transform:scaleX(1)}.marketplace-toolbar input:focus{border-color:rgba(201,168,76,.6)!important;box-shadow:0 0 0 3px rgba(201,168,76,.08)}@media(max-width:760px){.marketplace-header{padding:0 16px!important}.marketplace-header img{height:34px!important}.marketplace-brand-sub{display:none}.marketplace-hero-inner{padding:52px 20px 30px!important}.marketplace-hero-inner h1{font-size:clamp(38px,12vw,58px)!important}.marketplace-toolbar{padding:14px 16px!important;flex-wrap:wrap}.marketplace-search{flex-basis:100%}.mobile-filter-btn{display:block!important;background:transparent;color:#F7F4EE;border:1px solid rgba(201,168,76,.3);padding:10px 13px;border-radius:7px;font-family:inherit;font-size:10px}.marketplace-toolbar label{margin-left:auto}.marketplace-workspace{display:block!important}.marketplace-filters{display:none}.marketplace-results{padding:28px 16px!important}.marketplace-results-head{align-items:flex-start!important;flex-direction:column}.marketplace-standard{border-left:0!important;border-top:1px solid rgba(201,168,76,.2);padding:10px 0 0!important}.grid{grid-template-columns:1fr!important}.compareBar{left:10px!important;right:10px!important;bottom:10px!important;flex-wrap:wrap}.compareNames{order:3;flex-basis:100%}.compareGrid{grid-template-columns:1fr!important}}
`