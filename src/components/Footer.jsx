...
export default function Footer() {
  ...
  return (
    <>
      <style>{`
        footer.vi-footer { background: var(--dark-mid); border-top: 1px solid rgba(201,168,76,.15); padding: clamp(48px,6vw,80px) var(--pad) 32px; }
        .uf-grid { display: grid; grid-template-columns: 2fr 1fr 1fr 1fr; gap: clamp(24px,4vw,56px); margin-bottom: clamp(32px,4vw,52px); }
        ...
        .uf-bottom { display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px; padding-top: 24px; border-top: 1px solid rgba(255,255,255,.05); font-size: 11px; font-weight: 300; color: rgba(247,244,238,.2); letter-spacing: .04em; }
        @media (max-width: 980px) { .uf-grid { grid-template-columns: 1fr 1fr; gap: 28px; } .uf-brand { grid-column: 1 / -1; } }
        @media (max-width: 640px) { .uf-grid { grid-template-columns: 1fr; } .uf-bottom { flex-direction: column; text-align: center; } }
      `}</style>
      ...
      <footer className="vi-footer">
        <div className="container">
          <div className="uf-grid">
            ...
            <div>
              ...
              <ul className="uf-links">
                <li><a href={BRAND.assessmentUrl} target="_blank" rel="noopener noreferrer">VALU Index</a></li>
            ...
            <div>
              ...
              <ul className="uf-links">
                <li><a href={BRAND.assessmentUrl} className="uf-cta" target="_blank" rel="noopener noreferrer">Take the VALU Snapshot &rarr;</a></li>
                {authChecked && (
                  user ? (
                    <>
          ...
          <div className="uf-bottom">
            <span>&copy; {new Date().getFullYear()} {BRAND.copyrightEntity} &middot; {BRAND.name} &middot; {BRAND.location}</span>
            <span>{BRAND.compliance}</span>
            <span><a href={`mailto:${BRAND.email}`} style={{ color: 'rgba(247,244,238,.2)', textDecoration: 'none' }}>{BRAND.email}</a></span>
          </div>
        </div>
      </footer>
    </>
