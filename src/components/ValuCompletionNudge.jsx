...
export default function ValuCompletionNudge() {
  ...
  useEffect(() => {
    let active = true
    async function load() {
      try { if (sessionStorage.getItem('valu_nudge_dismissed_today') === new Date().toISOString().slice(0,10)) return } catch {}
      ...
      if (token) {
        const handoff = await fetch('/api/valu-handoff', { headers:{ Authorization:`Bearer ${token}` } }).catch(()=>null)
  ...
  if (!show) return null
  function dismiss() { try { sessionStorage.setItem('valu_nudge_dismissed_today', new Date().toISOString().slice(0,10)) } catch {}; setShow(false) }
  return <div style={S.overlay} role="dialog" aria-modal="true" aria-label="Complete your VALU Index"><div style={S.card}><button onClick={dismiss} aria-label="Close" style={S.close}>×</button><div style={S.eyebrow}>YOUR PROFILE IS INCOMPLETE</div><h2 style={S.title}>Your Basic listing is live.</h2><p style={S.copy}>Complete the full VALU assessment to establish your official VALU Index and move your professional profile forward.</p><a href={url} style={S.button}>COMPLETE FULL ASSESSMENT →</a><button onClick={dismiss} style={S.later}>Remind me later</button></div></div>
}
