...
function sessionListId(sessionId) {
  return process.env[`BREVO_SESSION_${sessionId}_LIST_ID`] || MASTER_LIST_ID
}
...
async function syncToBrevo({ email, fullName, role, organisation, whatsapp, session }) {
  ...
  // Custom attributes are sent only when explicitly configured in Brevo.
  if (process.env.BREVO_CUSTOM_ATTRIBUTES_ENABLED === 'true') {
    attributes.ROLE = role || ''
    attributes.ORGANISATION = organisation || ''
    attributes.WHATSAPP = whatsapp || ''
    attributes.EVENT_NAME = `The Professional Standard Series — Session ${session.id}`
    attributes.EVENT_SESSION = session.id
    attributes.EVENT_TITLE = session.title
    attributes.EVENT_DATE = session.start
    attributes.REGISTRATION_SOURCE = `professional_standard_session_${session.id}`
  }
...
export async function POST(request) {
  try {
    ...
    const { error } = await supabase.from('professional_standard_event_registrations').insert({ session_id: sessionId, full_name: fullName, email, role: role || null, organisation: organisation || null, whatsapp: whatsapp || null })
    if (error?.code === '23505') return Response.json({ error: 'This email is already registered for this session.' }, { status: 409 })
    if (error) { console.error('Session registration insert error:', error); return Response.json({ error: 'Registration could not be saved. Please try again.' }, { status: 500 }) }
    ...
    try {
      ...
      return Response.json({ error: 'Your registration was saved, but we could not connect it to our event mailing system. Please try again or contact Valoria Institute.' }, { status: 502 })
    }
  } catch (error) {
