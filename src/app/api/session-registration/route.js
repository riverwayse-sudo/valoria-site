...
const sessions = Object.fromEntries(
  PROFESSIONAL_STANDARD_SERIES
    .filter((session) => !session.replay)
...
export async function POST(request) {
  try {
    ...
    const { error } = await supabase.from('professional_standard_event_registrations').insert({
      session_id: sessionId, full_name: fullName, email, role: role || null, organisation: organisation || null,
    })
    ...
    syncRegistration(email, fullName, role, organisation, session).catch((err) => console.error('Registration notification error:', err))
