...
// allowed — it never governs whether a FILLED-IN answer is well-formed.
// These catch people typing junk into a field just to get past it: numbers
// in a location field, letters in a years-of-experience field, a non-URL
// in a link field, etc. Empty string always passes here — required-ness
// is checked separately — so these only fire once something's been typed.
const VALIDATORS = {
  place: (v) => {
    const t = (v || '').trim()
    if (!t) return true
...
function ScreenBody(props) {
  ...
  switch (screen.kind) {
    ...
    case 'text': {
      ...
      const isValid = (!screen.required || trimmed) && formatOk
