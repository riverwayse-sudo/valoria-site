  ...
  return (
    <>
      ...
      <main className="events-page">
        ...
        <section className="events-list"><div className="events-wrap"><div className="events-list-head"><div><span>UPCOMING</span><h2>What is next.</h2></div><p>Event dates are published when the Valoria calendar is confirmed.</p></div>
          <div className="events-grid">
            {PROFESSIONAL_STANDARD_SERIES.filter(s => !s.replay).map(session => (
              <article className="event-card" id={`session-${session.id}`} key={session.id}>
                {session.flyerPosition && <div className="event-card-flyer"><img src={EVENT_FLYER_SPRITE} alt={`${session.title} flyer`} style={{ transform: session.flyerPosition === 'right' ? 'translateX(-50%)' : 'translateX(0)' }} /></div>}
                ...
                <div className="event-card-meta"><strong>{session.dateApproved ? new Intl.DateTimeFormat('en-US', { timeZone: 'Africa/Lagos', month: 'long', day: '2-digit', year: 'numeric' }).format(new Date(session.start)) : 'DATE TO BE CONFIRMED'}</strong><span>VIRTUAL · 90 MINUTES{session.speaker ? ` · ${session.speaker}` : ''}</span></div>
              </article>
            ))}
          </div>
        </div></section>
