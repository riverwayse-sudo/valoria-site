  ...
  async function handleSignup() {
    ...
    try {
      ...
      if (userId) {
        ...
        // NOTE: previously this call had no .ok check and a bare
        // .catch(() => {}) — if the insert failed (e.g. an RLS policy
        // blocking the anon key), the person would still see a normal
        // success screen while their marketplace listing silently never
        // got created. Now we check response.ok and log failures so they
        // show up in Vercel Function logs instead of vanishing entirely.
        try {
          const listingRes = await fetch(`${SUPABASE_URL}/rest/v1/professional_profiles`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
              "apikey": SUPABASE_ANON_KEY,
          ...
          if (!listingRes.ok) {
            ...
            console.error("professional_profiles auto-insert failed:", listingRes.status, errText);
          }
        } catch (listingErr) {
          console.error("professional_profiles auto-insert threw:", listingErr);
        }
      }
